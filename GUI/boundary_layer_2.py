...
def print_results(x, y,z,w):
    ...
    """
    result_text = "Reynolds number (-) = {:4.0f} \n\nBL thickness (mm) = {:2.5f}\n\nFirst cell height (mm) = {:2.5f}\n\nCells numbers = {:2.0g}".format(x, y,z,w)
...
if __name__ == "__main__":
    ...
    root.geometry("380x502")
    root.resizable(width=False, height=False)
    #frames and widgets inside the root window are properly distributed
    ...
    #subframes
    fr1 = tk.LabelFrame(root, text="Inputs", bd=2, relief=tk.GROOVE)
    fr1.grid(row=0, column=0, padx=10, pady=10, sticky="ew")
    fr2 = tk.LabelFrame(root, text="Results", bd=2, relief=tk.GROOVE)
    fr2.grid(row=1, column=0, padx=10, pady=10, sticky="ew")
    
    fr1.grid_columnconfigure(0, weight=1)
    fr1.grid_columnconfigure(2, weight=1)
    fr2.grid_columnconfigure(0, weight=1)
    
    tk.Label(fr1, text="Fluid:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
    flu_set = ttk.Combobox(fr1, values=flu_opt, width=17)
    flu_set.grid(row=0, column=1, padx=10, pady=5)
    flu_set.set("Air")
    
    tk.Label(fr1, text="Temperature (K):").grid(row=1, column=0, padx=10, pady=5, sticky="w")
    t1 = tk.Entry(fr1)
    t1.grid(row=1, column=1, padx=10, pady=5)
    t1.insert(0, "300")

    tk.Label(fr1, text="Velocity (m/s):").grid(row=2, column=0, padx=10, pady=5, sticky="w")
    v1 = tk.Entry(fr1)
    v1.insert(0, "1.0")
    v1.grid(row=2, column=1, padx=10)

    tk.Label(fr1, text="Length Scale (mm):").grid(row=3, column=0, padx=10, pady=5, sticky="w")
    ls1 = tk.Entry(fr1)
    ls1.insert(0, "100.0")
    ls1.grid(row=3, column=1, padx=10, sticky="w")

    tk.Label(fr1, text="y+ (-):").grid(row=4, column=0, padx=10, pady=5, sticky="w")
    yplus1 = tk.Entry(fr1)
    yplus1.insert(0, "1")
    yplus1.grid(row=4, column=1, padx=10)

    tk.Label(fr1, text="layer ratio:").grid(row=5, column=0, padx=10, pady=5, sticky="w")
    ...
    # Corrected button placement using grid
    custom_font = ("Helvetica", 12, "bold")

    btn_calculate = ttk.Button(root, text="Calculate", command=calculation, width=20, style="TButton")
    btn_calculate.grid(row=2, column=0, pady=2)

    btn_exit = ttk.Button(root, text="Exit", command=ex, width=20, style="TButton")
    btn_exit.grid(row=3, column=0, pady=4)
